...
export class Search {
  ...
  static searchInput(
    ...
    return this.applyFixedAttributes({ where: whereInput }, fixedAttributes);
  }

  // create a search input (where + order) from the search attributes
  static searchInputAdvanced(
    ...
    if (!searchAttributes?.length && !fixedAttributes?.length) resultSearch = null;

    return this.applyFixedAttributes(resultSearch, fixedAttributes);
  }

  // receives a search input (where + order) and a list of fixed attributes
  // applies the fixed attributes to the where input of the search input
  static applyFixedAttributes(searchInput: any, fixedAttributes: FixedAttribute[]): any {
    // if no fixed attributes, return the where input
    if (!fixedAttributes?.length) return searchInput;

    // create an AND object with the all fixed attributes
    const fixedAttributesAndObject = fixedAttributes?.reduce((and: any, fixedAttribute) => {
    ...
    // if searchInput is null or undefined, return the fixed attributes as where input
    if (!searchInput) return { where: fixedAttributesAndObject };

    // if searchInput where is null or undefined, add the fixed attributes as where input and return
    if (!searchInput.where) return { ...searchInput, where: fixedAttributesAndObject };

    if (fixedAttributes?.length) {
      // if searchInput where is an array, add the fixed attributes to each element
      if (Array.isArray(searchInput.where)) {
        searchInput.where = searchInput.where.map((where: any) => {
          return _.merge(where, fixedAttributesAndObject);
        });
      } else {
        // if searchInput where is an object, add the fixed attributes to the object
        searchInput.where = _.merge(searchInput.where, fixedAttributesAndObject);
      }
    }
